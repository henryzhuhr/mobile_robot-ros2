...
namespace manager
{
  ...
  private:
     ...
     * @brief 系统状态更新服务相关
     */
    rclcpp::Service<S_US>::SharedPtr update_state__services_;                            // 更新系统状态的服务
    const std::string update_state__services_name = SystemState::services::update_state; // 系统状态更新的服务名称
    void update_system_state__services_callback(const std::shared_ptr<S_US::Request> request, std::shared_ptr<S_US::Response> response);
