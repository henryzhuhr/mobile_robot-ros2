...
void BaseNode::UpdateState() {
    ...
    while (!this->update_state__client_->wait_for_service(std::chrono::seconds(1))) {
        ...
        if (!rclcpp::ok()) {
            RCLCPP_ERROR(this->get_logger(), "Interrupted while waiting for service[%s]...",
                         SystemState::services::update_state.c_str());
        }
        RCLCPP_WARN(this->get_logger(), "Waiting for the server[%s] to go online",
                    SystemState::services::update_state.c_str());
    }

    // Send an asynchronous request, then wait to return, and call the callback
