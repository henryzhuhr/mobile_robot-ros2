...
source $current_path/color.sh

echo "ROS2 builder info (run in $CURRENT_DIR) :"
echo "	- ${LGREEN}ROS_DISTRO${DEFAULT}=$ROS_DISTRO"
echo "	- ${LGREEN}ROS_PKG${DEFAULT}=$ROS_PKG"
echo "	- ${LGREEN}ROS_ROOT${DEFAULT}=$ROS_ROOT"
...
sudo cp build_tmp/ros-archive-keyring.gpg /usr/share/keyrings/ros-archive-keyring.gpg
...
# install dependencies using rosdepc
sudo rosdepc init
rosdepc update
rosdepc install -y \
	--ignore-src \
