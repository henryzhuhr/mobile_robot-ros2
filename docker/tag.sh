...
if [ $ARCH = "aarch64" ]; then
	...
	{	
		echo "$ERROR"
		echo  "${LRED}Docker containers aren't supported on Jetson prior to JetPack 4.4 / L4T R32.4.3${DEFAULT}"
...
if [ -n "$ROS_DISTRO" ]; then
	...
	CONTAINER_IMAGE="ros:$ROS_DISTRO-desktop-l4t-$CONTAINER_TAG"
else
